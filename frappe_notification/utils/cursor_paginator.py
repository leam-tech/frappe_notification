...
class CursorPaginatorSortByDirection(Enum):
    ASC = "asc"
    DESC = "desc"
...
class CursorPaginatorSortByField(Enum):
    @classmethod
    def has_value(cls, value):
        return value in cls._value2member_map_
...
class CursorPaginatorSortBy(frappe._dict):
    direction: CursorPaginatorSortByDirection
    field: CursorPaginatorSortByField


class CursorPaginatorExecutionArgs(frappe._dict):
    before: Optional[str]
    after: Optional[str]
    first: Optional[int]
    last: Optional[int]
    sort_by: Optional[CursorPaginatorSortBy]
...
class CursorPaginator(object):
    ...
    def execute(self, args: CursorPaginatorExecutionArgs):
    ...
    def get_data(self, doctype, filters, fields, sorting_fields, sort_dir, limit):
    ...
    def get_sort_args(self, sorting_input: CursorPaginatorSortBy = None):
        ...
        if sorting_input and sorting_input.field:
            sort_dir = sorting_input.get("direction").value \
                if sorting_input.get("direction") else "asc"

            sorting_input_field = sorting_input.get("field").value
